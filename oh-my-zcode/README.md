...
Arms the scope **by invocation**: step 0 derives targets and environment from your argument (`localhost|127.x|*.local|192.168.x` → `local`/`dev`, `*.staging*` → `staging`, anything else asks you to confirm the env, `prod` → refused) and writes `active_scope.json` with a 60-minute expiry — no Settings, no forms; the run expires on its own. The environment gate comes first: test/dev or nothing. 🟣 `ohmy-plan-critic` checks the prize plan against the doctrine before anything is dispatched. The beasts are dispatched attackers that receive target and objective only — the cage (the `PreToolUse` scope gate, token-matched attack commands and tagged red-team subagent dispatches via Agent/Task: env ≠ prod, matching session, matching hosts) enforces every constraint, not their own restraint. Impact is proven by sample or by controlled callback. 🟠 `ohmy-finding-verifier` re-executes each demonstrated impact blind and re-verifies cleanup. Recon's `surface.md` opens with a `## PROVENANCE` block (artifact tested, source commit, working-tree state — raw command outputs), and every structured entry carries a claim type: `behavior` or `source`. Step 2.5 reads the target's own repository and writes `source-map.md` — declared intentions per endpoint and guard diffs, each divergence a pre-chewed hypothesis — or records `no source available` and the run goes black-box. Waves cascade — each wave's loot re-dispatched as the next wave's hypotheses — until a prize is proven or two consecutive waves add no new capability; the report ends with a detection report per prize (x launched / y blocked / z logged). The cage is a guardrail, not an authorization oracle — hooks see main-session commands and dispatches, not subagent internals; a hostile agent could still forge the `active_scope.json` file itself; the bar is raised and the residual risk documented — it raises the bar mechanically, it does not replace the operator's judgment.
