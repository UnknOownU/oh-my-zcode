...
Delegate to `source-verifier`. Pass it the draft and nothing else — it opens every source itself, because the draft's summary of a source is exactly what it was called to check.
...
- `CITATIONS REVISE` → dispatch a NEW `draft-writer` with the corrections to apply to the file, then send it back. **Three rounds maximum.**
...
4. **The orchestrator never writes the report.** The draft is produced by `draft-writer`; corrections are routed back to a new writer dispatch, never applied by hand.
