...
Build the attack plan by WSTG angle families. Delegate to `plan-critic` (three rounds maximum). It answers `PLAN READY` or `PLAN REVISE` — that vocabulary only; never ask it for a VERDICT.
...
Delegate the candidates to `finding-verifier` WITHOUT the attacker's reasoning. You get per finding CONFIRMED / NOT REPRODUCED / OUT OF SCOPE / PROOF-TYPE MISMATCH, each with the raw output of the re-execution.
