...
exports.splitIdentifierSegments = splitIdentifierSegments;
exports.normalizeProseWord = normalizeProseWord;
exports.extractProseCandidates = extractProseCandidates;
exports.segmentLookupVariants = segmentLookupVariants;
...
function splitIdentifierSegments(name) {
    ...
    for (const run of name.match(/[\p{L}\p{N}]+/gu) ?? []) {
        ...
        for (const part of parts) {
            if (out.size >= MAX_SEGMENTS_PER_NAME)
                return [...out];
            const seg = part.toLowerCase();
            if (seg.length < MIN_SEGMENT_CHARS || seg.length > MAX_SEGMENT_CHARS)
                continue;
            if (/^\p{N}+$/u.test(seg))
                continue;
            out.add(seg);
 ...
 */
function normalizeProseWord(word) {
 ...
 */
function extractProseCandidates(prompt) {
    if (!prompt)
        return [];
    const seen = new Set();
    for (const run of prompt.match(/[\p{L}\p{N}]+/gu) ?? []) {
        if (seen.size >= MAX_PROSE_CANDIDATES)
            break;
        if (run.length > MAX_PROSE_CHARS)
            continue;
        const w = normalizeProseWord(run);
        if (w.length < MIN_PROSE_CHARS || w.length > MAX_PROSE_CHARS)
            continue;
        if (/^\p{N}+$/u.test(w))
            continue;
        if (ENGLISH_PROSE_STOPWORDS.has(w))
            continue;
        seen.add(w);
    }
    return [...seen];
 ...
 */
function segmentLookupVariants(word) {
    const variants = [word];
...
//# sourceMappingURL=identifier-segments.js.map