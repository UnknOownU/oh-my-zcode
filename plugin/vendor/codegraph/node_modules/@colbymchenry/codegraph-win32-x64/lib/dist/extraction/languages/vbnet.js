...
function hasModifier(node, re) {
    for (let i = 0; i < node.childCount; i++) {
        const child = node.child(i);
        if (child?.type === 'member_modifier' && re.test(child.text))
            return true;
    }
    return false;
}
 ...
 */
function extractVbnetReturnType(node, source) {
...
exports.vbnetExtractor = {
    ...
    getVisibility: (node) => {
        if (hasModifier(node, /^private$/i))
            return 'private';
        if (hasModifier(node, /^protected(\s+friend)?$/i))
            return 'protected';
        if (hasModifier(node, /^friend$/i))
            return 'internal';
    ...
    extractImport: (node, source) => {
        ...
        const nameNode = [...node.namedChildren]
            .reverse()
            .find((c) => c.type === 'qualified_name' || c.type === 'simple_name' || c.type === 'global_qualified_name' || c.type === 'identifier');
        if (nameNode) {
            return { moduleName: (0, tree_sitter_helpers_1.getNodeText)(nameNode, source), signature: importText };
        }
        return null;
    },
    visitNode: (node, ctx) => {
        ...
        if (node.type === 'event_declaration' || node.type === 'custom_event_declaration') {
            ...
            if (nameNode) {
                ctx.createNode('field', (0, tree_sitter_helpers_1.getNodeText)(nameNode, ctx.source), node);
            }
            return true;
        }
