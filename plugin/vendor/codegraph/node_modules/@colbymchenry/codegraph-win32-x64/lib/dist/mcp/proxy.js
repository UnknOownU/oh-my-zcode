...
function sendClientHello(socket) {
    const clientHello = {
        codegraph_client: 1,
        pid: process.pid,
        hostPid: parseHostPpid(process.env[wasm_runtime_flags_1.HOST_PPID_ENV]) ?? early_ppid_1.EARLY_PPID,
    };
    try {
...
async function runLocalHandshakeProxy(deps) {
    ...
    const routeToDaemon = (line) => {
        if (daemonStatus === 'ready' && daemonSocket) {
            ...
            if (process.env.CODEGRAPH_MCP_DEBUG)
                process.stderr.write(`[mcp-debug] proxy->daemon ${line.slice(0, 80)}\n`);
            try {
            ...
            if (process.env.CODEGRAPH_MCP_DEBUG)
                process.stderr.write(`[mcp-debug] proxy-buffer(${daemonStatus}) ${line.slice(0, 80)}\n`);
            pending.push(line);
        }
    };
    // ---- client (stdin) ----
    ...
    process.stdin.on('data', (chunk) => {
        ...
        while ((idx = stdinBuf.indexOf('\n')) !== -1) {
            ...
            if (msg.method === 'initialize') {
                ...
                writeClient({ jsonrpc: '2.0', id: msg.id, result: { protocolVersion: session_1.PROTOCOL_VERSION, capabilities: { tools: {} }, serverInfo: session_1.SERVER_INFO, instructions: (0, session_1.initializeInstructions)(server_instructions_1.SERVER_INSTRUCTIONS) } });
    ...
    (0, startup_handshake_1.armStartupHandshakeTimeout)(() => {
        process.stderr.write('[CodeGraph MCP] No MCP traffic since startup; assuming an abandoned launch and shutting down (#1185). ' +
    ...
    if (socket && !socket.destroyed && !shuttingDown) {
        ...
        socket.on('data', (chunk) => {
            ...
            while ((idx = sockBuf.indexOf('\n')) !== -1) {
                ...
                if (process.env.CODEGRAPH_MCP_DEBUG)
                    process.stderr.write(`[mcp-debug] daemon->proxy ${line.slice(0, 80)}\n`);
        ...
        for (const line of pending) {
            trackInflight(line);
            if (process.env.CODEGRAPH_MCP_DEBUG)
                process.stderr.write(`[mcp-debug] proxy-flush ${line.slice(0, 80)}\n`);
            try {
                socket.write(line + '\n');
            }
            catch { /* ignore */ }
