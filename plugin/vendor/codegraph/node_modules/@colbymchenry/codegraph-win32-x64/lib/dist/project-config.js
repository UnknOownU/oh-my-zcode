...
exports.loadIncludePatterns = loadIncludePatterns;
exports.clearProjectConfigCache = clearProjectConfigCache;
exports.addIncludeIgnoredPatterns = addIncludeIgnoredPatterns;
/**
 ...
 */
function extractInclude(parsed, file) {
    const raw = parsed.include;
    if (raw === undefined)
        return [];
    if (!Array.isArray(raw)) {
        (0, errors_1.logWarn)(`Ignoring "include" in ${exports.PROJECT_CONFIG_FILENAME}: must be an array of gitignore-style patterns`, { file });
        return [];
    }
    const out = [];
    for (const entry of raw) {
        if (typeof entry !== 'string' || !entry.trim()) {
            (0, errors_1.logWarn)(`Ignoring an "include" entry in ${exports.PROJECT_CONFIG_FILENAME}: every pattern must be a non-empty string`, { file });
            continue;
        }
        out.push(entry.trim());
    }
 ...
 */
function loadIncludePatterns(rootDir) {
 ...
 */
function addIncludeIgnoredPatterns(rootDir, patterns) {
    const file = path.join(rootDir, exports.PROJECT_CONFIG_FILENAME);
    let config = {};
    let raw = null;
    try {
    ...
    if (raw !== null) {
        ...
        try {
            ...
            throw new Error(`${exports.PROJECT_CONFIG_FILENAME} is not valid JSON — fix it by hand, then re-run.`);
        }
    ...
    const existing = Array.isArray(config.includeIgnored)
        ? config.includeIgnored.filter((p) => typeof p === 'string')
        : [];
