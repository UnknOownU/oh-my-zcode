 ...
 */
function readOhpmMain(projectRoot, dirRel) {
    let parsed;
    try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        parsed = require('jsonc-parser').parse(fs.readFileSync(path.join(projectRoot, dirRel, OHPM_MANIFEST), 'utf-8'));
    }
    catch {
        return null;
    }
...
function collectOhpmFileDeps(projectRoot) {
    const byName = new Map();
    ...
    while (queue.length > 0) {
        ...
        if (++visited > OHPM_WALK_DIR_BUDGET)
            break;
        ...
        for (const e of entries) {
            if (e.isDirectory()) {
                if (depth >= OHPM_WALK_MAX_DEPTH)
                    continue;
                if (e.name.startsWith('.') || OHPM_SKIP_DIRS.has(e.name))
                    continue;
                queue.push({ rel: rel ? `${rel}/${e.name}` : e.name, depth: depth + 1 });
                continue;
            }
            if (e.name !== OHPM_MANIFEST)
                continue;
...
function readOhpmFileDeps(manifestAbs) {
    const out = [];
    let parsed;
    try {
