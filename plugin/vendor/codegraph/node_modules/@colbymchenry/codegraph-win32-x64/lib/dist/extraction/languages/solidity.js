...
exports.solidityExtractor = void 0;
 ...
 */
function getInheritanceAncestors(node, source) {
    const ancestors = [];
    for (let i = 0; i < node.namedChildCount; i++) {
        const child = node.namedChild(i);
        if (!child || child.type !== 'inheritance_specifier')
            continue;
        const ancestor = (0, tree_sitter_helpers_1.getChildByField)(child, 'ancestor');
        if (!ancestor)
            continue;
        ...
        if (id.length > 0) {
            ...
            ancestors.push((0, tree_sitter_helpers_1.getNodeText)(last, source));
        }
    }
    return ancestors;
}
function fallbackReceiveName(node) {
    ...
    for (let i = 0; i < node.childCount; i++) {
        ...
        if (!child)
            continue;
        const t = child.text;
        if (t === 'fallback' || t === 'receive')
            return t;
    }
    return 'fallback';
}
exports.solidityExtractor = {
    ...
    resolveName: (node, _source) => {
        ...
        if (node.type === 'fallback_receive_definition')
            return fallbackReceiveName(node);
        return undefined;
    },
    getSignature: (node, source) => {
        ...
        for (let i = 0; i < node.namedChildCount; i++) {
            ...
            if (child.type === 'parameter' && fieldName !== 'return_type') {
                params.push((0, tree_sitter_helpers_1.getNodeText)(child, source));
            }
        ...
        if (visibility)
            parts.push((0, tree_sitter_helpers_1.getNodeText)(visibility, source));
        if (mutability)
            parts.push((0, tree_sitter_helpers_1.getNodeText)(mutability, source));
        if (returnType)
            parts.push((0, tree_sitter_helpers_1.getNodeText)(returnType, source));
        return parts.join(' ');
    },
    getVisibility: (node) => {
    ...
    visitNode: (node, ctx) => {
        ...
        if (t === 'contract_declaration' ||
            ...
            const body = (0, tree_sitter_helpers_1.getChildByField)(node, 'body');
            if (!nameNode)
                return false;
        ...
        if (t === 'enum_value') {
            ctx.createNode('enum_member', (0, tree_sitter_helpers_1.getNodeText)(node, ctx.source), node);
            return true;
        }
        ...
        if (t === 'event_definition') {
            ...
            ctx.createNode('field', name, node, {
                signature: (0, tree_sitter_helpers_1.getNodeText)(node, ctx.source).trim().slice(0, 200),
            });
            return true;
        }
    ...
    extractImport: (node, source) => {
        ...
        const sourceField = (0, tree_sitter_helpers_1.getChildByField)(node, 'source');
        if (!sourceField)
            return null;
        ...
        if (stringContent.length > 0) {
            moduleName = (0, tree_sitter_helpers_1.getNodeText)(stringContent[0], source);
        }
        else {
            moduleName = (0, tree_sitter_helpers_1.getNodeText)(sourceField, source);
        }
