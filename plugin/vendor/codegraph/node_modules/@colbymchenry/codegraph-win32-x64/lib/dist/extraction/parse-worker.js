...
worker_threads_1.parentPort.on('message', async (msg) => {
    if (msg.type === 'load-grammars') {
        ...
        await (0, grammars_1.loadGrammarsForLanguages)(msg.languages, msg.grammarBuffers);
        ...
        try {
            ...
            let result;
            const frameworksNeedDecode = frameworkNames && frameworkNames.length > 0
                ? (0, frameworks_1.getApplicableFrameworks)((0, frameworks_1.getAllFrameworkResolvers)().filter((r) => frameworkNames.includes(r.name)), language).some((fw) => !!fw.extract)
                : false;
            if (!frameworksNeedDecode) {
                const raw = (0, kernel_1.tryKernelExtractRaw)(filePath, content, language);
                if (raw) {
                    result = {
                        nodes: [],
                        edges: [],
            ...
            result ??= (0, tree_sitter_1.extractFromSource)(filePath, content, language, frameworkNames);
            ...
            worker_threads_1.parentPort.postMessage({ type: 'parse-result', id, result, parseMs: performance.now() - t0 });
        }
        catch (err) {
