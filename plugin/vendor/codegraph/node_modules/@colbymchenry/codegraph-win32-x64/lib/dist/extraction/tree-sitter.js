...
class TreeSitterExtractor {
    ...
    sourceIsPreParsed = false;
    constructor(filePath, source, language, options) {
     ...
     */
    composeReceiverQualifiedName(receiverType, name) {
        ...
        if (nameField && objectField && (node.type === 'method_invocation' || node.type === 'member_call_expression' || node.type === 'scoped_call_expression')) {
            ...
            if (this.language === 'cpp' && func) {
                ...
                for (let i = 0; i < node.namedChildCount; i++) {
                    ...
                    if (child?.type !== 'ERROR')
                        continue;
                    const op = child.namedChildren.find((c) => c.type === 'operator_name');
                    if (op) {
                        operatorName = (0, tree_sitter_helpers_1.getNodeText)(op, this.source);
                        break;
                    }
                }
                if (operatorName) {
                    ...
                    if (/^[^\w\s]/.test(sym))
                        operatorName = `operator${sym.replace(/\s+/g, '')}`;
                    ...
                    const receiver = (0, tree_sitter_helpers_1.getNodeText)(func, this.source).replace(/->/g, '.').replace(/\s+/g, '');
...
function extractFromSource(filePath, source, language, frameworkNames) {
    ...
    if (detectedLanguage === 'svelte') {
        ...
        const kernelResult = (0, kernel_1.tryKernelExtract)(filePath, source, detectedLanguage);
        if (kernelResult) {
            result = kernelResult;
        }
        else {
            ...
            const deferredPre = (0, kernel_1.takeDeferredPreParse)(filePath, source, detectedLanguage);
            const extractor = new TreeSitterExtractor(filePath, deferredPre ?? source, detectedLanguage, { sourceIsPreParsed: deferredPre != null });
            result = extractor.extract();
        }
    }
