...
const beta_signup_1 = require("./beta-signup");
...
async function runInstallerWithOptions(opts) {
    ...
    if (!useDefaults && installedIds.length > 0) {
        await (0, beta_signup_1.maybeOfferBetaSignup)({ source: 'cli-install' });
    }
...
function refreshTargets(targets, location) {
    return targets.map((target) => {
        ...
        return {
            ...base,
            status: changedPaths.length > 0 ? 'refreshed' : 'unchanged',
            changedPaths,
        };
    });
}
...
async function runUninstaller(opts) {
    ...
    if (location === 'global' && opts.keepCli !== true && opts.cliFilename) {
        const { planBinaryRemoval, executeBinaryRemoval, defaultProbes } = await Promise.resolve().then(() => __importStar(require('../upgrade/remove-binary')));
        ...
        if (plan.summary.length > 0) {
            ...
            if (removeBinaries) {
                ...
                for (const p of result.removed)
                    clack.log.success(`Removed ${tildify(p)}`);
