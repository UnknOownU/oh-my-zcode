 ...
 */
function resolveParseTimeoutMs(envVal) {
...
class ParseWorkerPool {
    ...
    prewarm() {
        while (this.workers.size < this.maxSize) {
            ...
            this.spawnOne();
            if (this.workers.size === before)
                break; // spawn failed / breaker tripped
        }
    }
    ...
    settle(job, result, err) {
        ...
        if (job.hardKillTimer)
            clearTimeout(job.hardKillTimer);
        if (err)
            job.reject(err);
        else
            job.resolve(result);
