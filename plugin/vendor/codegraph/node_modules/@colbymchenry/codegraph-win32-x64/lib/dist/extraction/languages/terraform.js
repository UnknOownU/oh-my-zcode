...
exports.terraformExtractor = void 0;
...
function stringLitValue(node, source) {
    const literal = node.namedChildren.find((c) => c?.type === 'template_literal');
    if (literal)
        return (0, tree_sitter_helpers_1.getNodeText)(literal, source);
...
function readBlockHeader(block, source) {
    const named = block.namedChildren.filter((c) => c !== null);
    const first = named[0];
    if (!first || first.type !== 'identifier')
        return null;
    const type = (0, tree_sitter_helpers_1.getNodeText)(first, source);
    const labels = [];
    for (let i = 1; i < named.length; i++) {
        ...
        if (child.type === 'string_lit') {
            ...
            labels.push((0, tree_sitter_helpers_1.getNodeText)(child, source));
        }
        else {
            break;
        }
 ...
 */
function collectReferences(expr, source, onRef) {
...
function emitRefFromVariableExpr(varExpr, source, onRef) {
    const id = varExpr.namedChildren.find((c) => c?.type === 'identifier');
    if (!id)
        return;
    const head = (0, tree_sitter_helpers_1.getNodeText)(id, source);
    if (BUILTIN_HEADS.has(head) || BUILTIN_KEYWORDS.has(head))
        return;
    ...
    const attrs = [];
    let cursor = varExpr.nextNamedSibling;
    while (cursor) {
        if (cursor.type === 'get_attr') {
            const attrId = cursor.namedChildren.find((c) => c?.type === 'identifier');
            if (!attrId)
                break;
            attrs.push((0, tree_sitter_helpers_1.getNodeText)(attrId, source));
    ...
    for (const qname of qualifyReference(head, attrs))
        onRef(qname, line, col);
}
function qualifyReference(head, attrs) {
    switch (head) {
        case 'var':
            // var.X — variable "X"
        ...
        case 'module': {
            ...
            if (attrs[1])
                refs.push(`module.${attrs[0]}:output.${attrs[1]}`);
            ...
            if (attrs[1] === 'outputs' && attrs[2])
                refs.push(`module.${attrs[0]}:remote-output.${attrs[2]}`);
            return refs;
        }
        case 'data':
            ...
            if (node.type === 'attribute' &&
                ...
                if (idNode && fileNodeId) {
                    ctx.addUnresolvedReference({
                        ...
                        referenceName: `var.${(0, tree_sitter_helpers_1.getNodeText)(idNode, ctx.source)}`,
 ...
 */
function emitModuleWiring(moduleName, block, body, ctx, fromNodeId) {
    for (const attr of body.namedChildren) {
        ...
        if (!idNode)
            continue;
        const attrName = (0, tree_sitter_helpers_1.getNodeText)(idNode, ctx.source);
        if (attrName === 'source') {
        ...
        if (MODULE_META_ARGS.has(attrName))
            continue;
        ctx.addUnresolvedReference({
            fromNodeId,
...
function describeBlock(type, labels) {
...
function emitLocals(body, ctx) {
    ...
    for (const attr of body.namedChildren) {
        ...
        if (!idNode)
            continue;
        const name = (0, tree_sitter_helpers_1.getNodeText)(idNode, ctx.source);
...
function emitReferencesInBody(body, ctx, fromNodeId, opts) {
    const queue = [];
    for (const c of body.namedChildren) {
        ...
        if (opts?.skipTopAttrs && c.type === 'attribute') {
            ...
            if (id && opts.skipTopAttrs.has((0, tree_sitter_helpers_1.getNodeText)(id, ctx.source)))
                continue;
        }
        queue.push(c);
    }
    while (queue.length) {
        ...
        if (n.type === 'expression') {
            collectReferences(n, ctx.source, (qname, line, column) => {
                if (opts?.suppressScoped && qname.includes(':'))
                    return;
                ctx.addUnresolvedReference({
                    fromNodeId,
                    referenceName: qname,
 ...
 */
function readStringAttr(body, name, source) {
    for (const attr of body.namedChildren) {
        if (!attr || attr.type !== 'attribute')
            continue;
        const idNode = attr.namedChildren.find((c) => c?.type === 'identifier');
        if (!idNode || (0, tree_sitter_helpers_1.getNodeText)(idNode, source) !== name)
            continue;
        const expr = attr.namedChildren.find((c) => c?.type === 'expression');
        const lit = expr ? findStringLit(expr) : null;
 ...
 */
function emitProviderSelectionRef(body, ctx, fromNodeId) {
    for (const attr of body.namedChildren) {
        if (!attr || attr.type !== 'attribute')
            continue;
        const idNode = attr.namedChildren.find((c) => c?.type === 'identifier');
        if (!idNode || (0, tree_sitter_helpers_1.getNodeText)(idNode, ctx.source) !== 'provider')
            continue;
        const expr = attr.namedChildren.find((c) => c?.type === 'expression');
        if (!expr)
            return;
 ...
 */
function emitModuleProvidersRefs(body, ctx, fromNodeId) {
    for (const attr of body.namedChildren) {
        if (!attr || attr.type !== 'attribute')
            continue;
        const idNode = attr.namedChildren.find((c) => c?.type === 'identifier');
        if (!idNode || (0, tree_sitter_helpers_1.getNodeText)(idNode, ctx.source) !== 'providers')
            continue;
        ...
        while (queue.length) {
            ...
            if (n.type === 'object_elem') {
                const val = (0, tree_sitter_helpers_1.getChildByField)(n, 'val');
 ...
 */
function providerSelectionFromExpr(expr, source) {
    const queue = [expr];
    while (queue.length) {
        const n = queue.shift();
        if (n.type === 'variable_expr') {
            ...
            if (next?.type === 'get_attr') {
                ...
                if (!attrId || next.nextNamedSibling)
                    return null;
                return `${head}.${(0, tree_sitter_helpers_1.getNodeText)(attrId, source)}`;
            }
            return next ? null : head;
        }
        if (n.type === 'function_call' || n.type === 'conditional' || n.type === 'for_expr')
            return null;
        for (const c of n.namedChildren) {
            if (c)
                queue.push(c);
