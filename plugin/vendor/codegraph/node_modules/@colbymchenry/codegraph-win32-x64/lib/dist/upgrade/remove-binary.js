...
exports.planBinaryRemoval = planBinaryRemoval;
exports.defaultRemoveDeps = defaultRemoveDeps;
exports.executeBinaryRemoval = executeBinaryRemoval;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const os = __importStar(require("os"));
const child_process_1 = require("child_process");
const index_1 = require("./index");
function defaultProbes(filename) {
    return {
        filename,
        ...
        capture: (cmd, args) => {
            const res = (0, child_process_1.spawnSync)(cmd, args, { encoding: 'utf8', windowsHide: true, timeout: 30_000 });
...
function installDirCandidates(p) {
    const P = pathFor(p.platform);
    const dirs = [];
    const method = (0, index_1.detectInstallMethod)({
        filename: p.filename,
        platform: p.platform,
        cwd: p.cwd,
        exists: p.exists,
    });
    if (method.kind === 'bundle' && method.installDir)
        dirs.push(method.installDir);
    if (p.env.CODEGRAPH_INSTALL_DIR)
        dirs.push(p.env.CODEGRAPH_INSTALL_DIR);
    ...
    if (p.platform === 'win32') {
        if (p.env.LOCALAPPDATA)
            dirs.push(P.join(p.env.LOCALAPPDATA, 'codegraph'));
    }
    else {
...
function planBinaryRemoval(p) {
    ...
    const rootInv = (0, index_1.npmInvocation)(p.platform, ['root', '-g']);
    const rootRes = p.capture(rootInv.cmd, rootInv.args);
    if (rootRes && rootRes.code === 0) {
        const pkgDir = P.join(rootRes.stdout.trim(), index_1.NPM_PACKAGE);
        if (rootRes.stdout.trim() && p.exists(pkgDir)) {
            plan.npmGlobal = true;
            plan.npmRoot = rootRes.stdout.trim();
            plan.summary.push(`npm global package (${index_1.NPM_PACKAGE})`);
        }
    }
    return plan;
}
function defaultRemoveDeps() {
    return {
        ...
        run: (cmd, args) => {
            const res = (0, child_process_1.spawnSync)(cmd, args, { stdio: 'inherit', windowsHide: true, timeout: 120_000 });
 ...
 */
function moveLockedExeAside(dir, deps) {
    if (deps.platform !== 'win32')
        return null;
    const P = pathFor(deps.platform);
    const resolvedDir = P.resolve(dir) + P.sep;
    if (!P.resolve(deps.execPath).startsWith(resolvedDir))
        return null;
...
function executeBinaryRemoval(plan, deps = defaultRemoveDeps()) {
    const result = { removed: [], leftovers: [], npm: 'skipped' };
    const P = pathFor(deps.platform);
    ...
    if (plan.npmGlobal) {
        ...
        let moved = null;
        if (plan.npmRoot)
            moved = moveLockedExeAside(plan.npmRoot, deps);
        const inv = (0, index_1.npmInvocation)(deps.platform, ['uninstall', '-g', index_1.NPM_PACKAGE]);
...
//# sourceMappingURL=remove-binary.js.map