...
class Daemon {
    ...
    startLivenessTimers() {
        if (this.maxIdleMs > 0) {
            ...
            this.maxIdleTimer = setInterval(() => {
                if (this.backstopShouldExit(isProcessAlive))
     ...
     */
    backstopShouldExit(isAlive) {
        if (this.stopping || this.clients.size === 0)
            return false; // idle timer owns the no-client case
        if (Date.now() - this.lastActivityAt < this.maxIdleMs)
            return false; // still within the window
        this.reapDeadClients(isAlive);
        if (this.clients.size === 0)
            return false; // sweep cleared them — idle timer takes over
