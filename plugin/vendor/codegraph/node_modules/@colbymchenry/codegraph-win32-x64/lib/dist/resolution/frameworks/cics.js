 ...
 */
const transidIndexes = new WeakMap();
function buildIndex(context) {
    ...
    for (const node of dataNodes) {
        if (node.language !== 'cobol')
            continue;
        if (!TRANID_NAME_RE.test(node.name))
            continue;
        ...
        if (index.has(tx))
            continue; // first declaration wins; collisions are rare and ambiguous
        const moduleNode = context
            .getNodesInFile(node.filePath)
            .find((n) => n.kind === 'module' && n.language === 'cobol');
        if (moduleNode)
            index.set(tx, moduleNode.id);
    }
    return index;
}
exports.cicsResolver = {
    name: 'cics',
    languages: ['cobol'],
    detect(context) {
    ...
    resolve(ref, context) {
        if (!ref.referenceName.startsWith(TRANSID_REF_PREFIX))
            return null;
