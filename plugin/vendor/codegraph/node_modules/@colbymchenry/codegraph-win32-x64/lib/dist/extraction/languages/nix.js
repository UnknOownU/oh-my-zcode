...
exports.nixExtractor = void 0;
const tree_sitter_helpers_1 = require("../tree-sitter-helpers");
function unwrapVariableExpression(node) {
    if (node.type !== 'variable_expression')
        return node;
    return node.namedChild(0) ?? node;
}
function getCalleeName(node, source) {
    let current = node;
    ...
    if (current.type === 'identifier' || current.type === 'select_expression') {
        return (0, tree_sitter_helpers_1.getNodeText)(current, source).trim();
    }
    return null;
}
function getDirectCalleeName(node, source) {
    let funcNode = node.childForFieldName('function') || node.namedChild(0);
    if (!funcNode)
        return null;
    funcNode = unwrapVariableExpression(funcNode);
    return (0, tree_sitter_helpers_1.getNodeText)(funcNode, source).trim();
}
function isStaticProjectPath(value) {
    return ((value.startsWith('./') || value.startsWith('../')) &&
        !/[\s{}()[\];"'<>$]/.test(value));
}
function getStaticImportPath(argNode, source) {
    let current = argNode;
    ...
    let text = (0, tree_sitter_helpers_1.getNodeText)(current, source).trim();
...
function isReturnedAttrsetMember(node) {
    let current = node;
    ...
    while (current) {
        ...
        if (parent.type === 'binding' && !current.equals(node))
            return false;
        if (parent.type === 'formal_parameters' || parent.type === 'formals')
            return false;
        if (parent.type === 'attrset' ||
...
function getCurriedParamsAndBody(node, source) {
    const params = [];
    let current = node;
...
function formatFunctionSignature(params) {
    if (params.length === 0)
        return '()';
    if (params.length > 1)
        return params.join(' : ');
...
function emitFileImport(ctx, importPath, anchorNode, source) {
    const impNode = ctx.createNode('import', importPath, anchorNode, {
        signature: (0, tree_sitter_helpers_1.getNodeText)(anchorNode, source).trim().slice(0, 100),
    });
...
exports.nixExtractor = {
    ...
    visitNode: (node, ctx) => {
        ...
        if (node.type === 'binding') {
            ...
            if (!attrpath)
                return false;
            const name = (0, tree_sitter_helpers_1.getNodeText)(attrpath, source).trim();
            if (!name)
                return false;
            ...
            if (valueNode.type === 'function_expression') {
                ...
                const initValue = (0, tree_sitter_helpers_1.getNodeText)(valueNode, source).slice(0, 100);
                ...
                if ((finalSegment === 'imports' || finalSegment === 'modules') && valueNode.type === 'list_expression') {
                    for (const child of valueNode.namedChildren) {
                        if (child.type === 'path_expression') {
                            const entryPath = (0, tree_sitter_helpers_1.getNodeText)(child, source).trim();
        ...
        if (node.type === 'inherit' || node.type === 'inherit_from') {
            ...
            if (attrs) {
                for (const child of attrs.namedChildren) {
                    const name = (0, tree_sitter_helpers_1.getNodeText)(child, source).trim();
                    if (name) {
        ...
        if (node.type === 'apply_expression') {
            ...
            const parentFn = node.parent?.type === 'apply_expression'
