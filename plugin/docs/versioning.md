...
> **Current source version: 3.0.0.** The installation repair stays on this exact version. It replaces the broken distributions, adds the universal package and corrects marketplace metadata and documentation. Build evidence records the source commit and archive checksums. The [published marketplace](https://unknoownu.github.io/oh-my-zcode/marketplace.json) is live and identifies the available universal package. All live native and universal marketplaces are named `unknoownu`; stable native aliases and versioned per-platform URLs remain available in [distribution and installation](distribution.md).
